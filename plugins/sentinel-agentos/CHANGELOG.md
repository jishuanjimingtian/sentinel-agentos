...
- **自定义规则** — 用户通过 `.agentos/rules.json` 追加危险命令、警告命令、敏感文件、保护文件
- **禁用内置规则** — `disabled` 数组按 key 禁用任意内置规则
- **优先级覆盖** — `overrideSeverity` 支持 warning↔dangerous 升降级
- **热加载** — 修改 rules.json 后 5 秒自动生效，无需重启
- **示例模板** — 启动时自动生成 `.agentos/rules.json` 模板

### 🔧 依赖

- sentinel-agentos: ^0.3.10 → ^0.3.12
